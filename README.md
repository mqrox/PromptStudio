...
PromptStudio は、生成AIプロンプトを考えるときに最初に使う、クリエイティブチームのための **プロンプト運用ワークベンチ** です。

社員が「こういう絵を作りたい」「この商品を動画にしたい」「この案件のトーンで広告案を出したい」と日本語で相談すると、必要に応じて意図や前提を深掘りします。そのうえで、会社の制作ルール、案件ナレッジ、モデル別ノウハウ、過去の成功・失敗例をもとに、実際に使えるプロンプトと生成設定へ整理します。

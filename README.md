...
> 日本語で「作りたいもの」を相談すると、意図を深掘りし、生成AIで使えるプロンプトと生成設定へ整理します。

PromptStudio は、プロンプト作成、保存、レビュー、共通スキル改善までをつなぐ、クリエイティブチームのための **プロンプト運用ワークベンチ** です。

生成AIサービスが増えるほど、プロンプトとノウハウは個人に散らばります。PromptStudio は、個人の試行錯誤を、会社で蓄積・改善できる制作力へ変えます。

社員は、会社の制作ルール、案件ナレッジ、モデル別ノウハウ、過去の成功・失敗例を使いながら、画像生成・動画生成・広告制作などに使えるプロンプトを作れます。

保存するのは、プロンプトだけではありません。依頼内容、参照情報、入力素材、生成設定、結果、レビューコメントまで残し、管理者が共通スキルを改善できる状態にします。

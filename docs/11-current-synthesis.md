...
最初の入口は Chrome拡張の対話UI。長期的な本体は PromptStudio Core であり、Chrome拡張、CLI、MCP server、Local API、将来のTeam Serverから同じskill、knowledge、artifact、レビューPDCAを使えるようにする。
...
ただし、AntigravityはPromptStudioの本体ではない。あくまで初期の有力な実行基盤である。PromptStudioの本質は、skill、knowledge、artifact、レビューPDCAであり、これらはAntigravity外でも再利用できる形式にする。
...
入口が違っても、会社として参照する前提条件、skill、knowledge、保存形式、レビューPDCAは同じにする。
...
- `docs/03-prompt-skill-pdca.md`: skill、knowledge、artifact保存、レビュー、PDCA
